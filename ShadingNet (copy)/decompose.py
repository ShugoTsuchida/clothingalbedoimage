...
def decompose_image_with_shadingnet(input_image):
    """ShadingNetを使用して画像を分解"""
    image_resized = cv2.resize(input_image, (image_height, image_width)).astype('float32')
...
def process_frame_decompose_first(input_image_path, output_dir, frame_num):
    ...
    # 1. ShadingNetで画像を分解
    albedo_image, shading_image = decompose_image_with_shadingnet(input_image)

    # 2. セグメンテーションによる人物領域の抽出
    try:
        clothing_mask = extract_clothing_region(input_image)
        person_segment = cv2.bitwise_and(input_image, input_image, mask=clothing_mask)
        albedo_segment = cv2.bitwise_and(albedo_image, albedo_image, mask=clothing_mask)
        ...
        cropped_original = person_segment[upper_body_y1:upper_body_y2, upper_body_x1:upper_body_x2]
        cropped_albedo = albedo_segment[upper_body_y1:upper_body_y2, upper_body_x1:upper_body_x2]
        ...
        cv2.imwrite(os.path.join(person_dir, f"frame_{frame_num:04d}_albedo.png"), cropped_albedo)
        cv2.imwrite(os.path.join(person_dir, f"frame_{frame_num:04d}_mask.png"), clothing_mask * 255)

        # 統計データの保存
        original_stats, r_orig, g_orig, b_orig, h_orig, l_orig = calculate_color_stats(cropped_original, mask=None)
        albedo_stats, r_alb, g_alb, b_alb, h_alb, l_alb = calculate_color_stats(cropped_albedo, mask=None)
